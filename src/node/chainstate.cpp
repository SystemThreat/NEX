                                    ...
                                    const ChainstateLoadOptions& options)
{
    ...
    // NEX: Initialize claim index (tracks BTC snapshot claims).
    // This is consensus-required: duplicate-claim and cap checks depend on it.
    if (!chainman.m_claim_index) {
        auto claimdb_path = chainman.m_options.datadir / "claimindex";
        chainman.m_claim_index = std::make_unique<CClaimIndex>(claimdb_path, 1 << 20);
        LogInfo("NEX claim index initialized at %s", fs::PathToString(claimdb_path));
    }
...
ChainstateLoadResult VerifyLoadedChainstate(ChainstateManager& chainman, const ChainstateLoadOptions& options)
{
    ...
    // NEX: Rebuild claim index if needed (wiped, missing, corrupted, or out of sync).
    // Uses strong integrity check: verifies best-block matches tip, totals are consistent.
    {
    const uint256 activeTip = [&]() -> uint256 {
        LOCK(cs_main);
        auto* tip = chainman.ActiveChain().Tip();
        return tip ? tip->GetBlockHash() : uint256{};
    }();

    if (chainman.m_claim_index && chainman.m_claim_index->NeedsRebuild(activeTip)) {
        LOCK(cs_main);
        const CChain& activeChain = chainman.ActiveChain();
        if (activeChain.Height() > 0) {
            LogInfo("NEX: Rebuilding claim index from active chain (height %d)...", activeChain.Height());
            chainman.m_claim_index->Clear();

            int claimsFound = 0;
            for (int h = 0; h <= activeChain.Height(); ++h) {
                CBlockIndex* pindex = activeChain[h];
                CBlock block;
                if (!chainman.m_blockman.ReadBlock(block, *pindex)) {
                    return {ChainstateLoadStatus::FAILURE,
                            _("Failed to read block during claim index rebuild")};
                }
                for (const auto& tx : block.vtx) {
                    if (!tx || !IsClaimTx(*tx)) continue;
                    ClaimData claim;
                    if (ParseClaimData(*tx, claim)) {
                        chainman.m_claim_index->WriteClaim(
                            claim.btcScriptPubKey, h, tx->GetHash().ToUint256());
                        claimsFound++;
                    }
                }
            }

            chainman.m_claim_index->SetBestBlock(activeChain.Tip()->GetBlockHash());
            LogInfo("NEX: Claim index rebuilt: %d claims found", claimsFound);
        } else {
            // Fresh chain — just mark as synced
            chainman.m_claim_index->SetBestBlock(uint256{});
        }
    }

    // Update claim index best-block on every startup to match chain tip
    if (chainman.m_claim_index) {
        LOCK(cs_main);
        const CChain& activeChain = chainman.ActiveChain();
        if (activeChain.Tip()) {
