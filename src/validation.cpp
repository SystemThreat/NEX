...
bool MemPoolAccept::PreChecks(ATMPArgs& args, Workspace& ws)
{
    ...
    // NEX claim transactions: full validation before mempool admission.
    // Don't relay claims that can never make a valid block.
    if (IsClaimTx(tx)) {
        // 1. Amount
        if (tx.vout[0].nValue != CLAIM_AMOUNT) {
            return state.Invalid(TxValidationResult::TX_CONSENSUS, "claim-wrong-amount");
        }

        // 2. Parse claim data
        ClaimData claim;
        if (!ParseClaimData(tx, claim)) {
            return state.Invalid(TxValidationResult::TX_CONSENSUS, "claim-parse-failed");
        }

        // 3. Parse claim message canonically
        auto parsed = ParseClaimMessageCanonical(claim.claimMessage);
        if (!parsed || parsed->version != 1 || parsed->snapshotBlock != 940000) {
            return state.Invalid(TxValidationResult::TX_CONSENSUS, "claim-message-invalid");
        }

        // 4. Deadline
        int64_t now = GetTime();
        if (now >= Consensus::CLAIM_DEADLINE_TIMESTAMP) {
            return state.Invalid(TxValidationResult::TX_CONSENSUS, "claim-expired");
        }

        // 5-6. Duplicate and cap checks happen in ConnectBlock (requires claim index).
        // Mempool does structural + cryptographic checks only.

        // 7. Merkle proof
        const auto& params = Params().GetConsensus();
        if (!VerifyClaimMerkleProof(claim.btcScriptPubKey, claim.snapshotBalanceSat,
                                    claim.merkleProof, params.claimMerkleRoot)) {
            return state.Invalid(TxValidationResult::TX_CONSENSUS, "claim-merkle-invalid");
        }

        // 8. BTC signature
        if (!VerifyClaimBTCSignature(claim.btcScriptPubKey, claim.btcSignature, claim.claimMessage)) {
            return state.Invalid(TxValidationResult::TX_CONSENSUS, "claim-sig-invalid");
        }

        // 9. Destination binding
        std::string decErr;
        CTxDestination signedDest = DecodeDestination(parsed->claimTo, decErr);
        if (!IsValidDestination(signedDest)) {
            return state.Invalid(TxValidationResult::TX_CONSENSUS, "claim-dest-invalid");
        }
        if (GetScriptForDestination(signedDest) != tx.vout[0].scriptPubKey) {
            return state.Invalid(TxValidationResult::TX_CONSENSUS, "claim-dest-mismatch");
        }

        return true; // Valid claim, relay it
    }
...
{
    // NEX Emission Schedule — 100 NEX/block, 375K blocks/era, 8 eras
    //
    // 20M genesis + 5M claims + 75M emission = 100M max supply
    //
    // Era 1: 100 NEX    Era 5: 6.25 NEX
    // Era 2:  50 NEX    Era 6: 3.125 NEX
    // Era 3:  25 NEX    Era 7: 1.5625 NEX
    // Era 4: 12.5 NEX   Era 8: 1.5625 NEX
    //
    // All eras divide exactly — zero remainder per era.
    // Block 3,000,000+: subsidy is zero. Mining ends October 2054.

    if (nHeight < 0) return 0;

    // Block 1 premine: 20M NEX bonus on top of era 1 subsidy.
    // Goes to whoever mines block 1 (solo pool payout address).
    // The genesis block coinbase is unspendable (Bitcoin Core design),
    // so the premine is delivered via block 1 s coinbase instead.
    const CAmount PREMINE = 2000000000000000LL; // 20,000,000 NEX in sats

    CAmount base = 0;
    for (int i = 0; i < Consensus::NUM_EMISSION_ERAS; ++i) {
        const auto& era = Consensus::EMISSION_TABLE[i];
        if (nHeight >= era.startHeight && nHeight <= era.endHeight) {
            base = era.baseSubsidy;
            break;
        }
    }

    if (nHeight == 1) return base + PREMINE;
    return base;
}
...
DisconnectResult Chainstate::DisconnectBlock(const CBlock& block, const CBlockIndex* pindex, CCoinsViewCache& view)
{
    ...
    // NEX: Undo any claim transactions in this block (reorg safety)
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
        const CTransaction& ctx = *(block.vtx[i]);
        if (IsClaimTx(ctx)) {
            ClaimData claim;
            if (ParseClaimData(ctx, claim) && m_chainman.m_claim_index) {
                m_chainman.m_claim_index->EraseClaim(claim.btcScriptPubKey);
            }
        }
    }
                               ...
                               CCoinsViewCache& view, bool fJustCheck)
{
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++)
    {
        ...
        // ── NEX Claim Transaction Handling ──────────────────────
        // Claim txs have null inputs (like coinbase) but version=3.
        // They mint 5 NEX from the claim pool, not from block subsidy.
        // Validate and record in the claim index.
        if (IsClaimTx(tx))
        {
            // Claim index is REQUIRED for consensus — fail hard if unavailable
            CClaimIndex* claimIdx = m_chainman.m_claim_index.get();
            if (!claimIdx) {
                state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "claim-index-unavailable",
                              "Claim index not initialized — cannot validate claim transactions");
                break;
            }
            CAmount totalClaimed = claimIdx->GetTotalClaimed();

            auto isAlreadyClaimed = [&](const std::vector<unsigned char>& spk) -> bool {
                return claimIdx ? claimIdx->IsClaimed(spk) : false;
            };

            std::string claimErr = ValidateClaimTx(
                tx,
                pindex->GetBlockTime(),
                totalClaimed,
                params.GetConsensus().claimMerkleRoot,
                isAlreadyClaimed
            );

            if (!claimErr.empty()) {
                state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, claimErr,
                              "claim validation failed: " + claimErr);
                break;
            }

            // Record the claim in the index
            if (claimIdx) {
                ClaimData claim;
                if (ParseClaimData(tx, claim)) {
                    claimIdx->WriteClaim(claim.btcScriptPubKey, pindex->nHeight, tx.GetHash().ToUint256());
                }
            }

            // Claim txs produce new coins (like coinbase) — add outputs to UTXO set
            CTxUndo undoDummy;
            UpdateCoins(tx, view, undoDummy, pindex->nHeight);
            continue;  // Skip normal input validation
        }
